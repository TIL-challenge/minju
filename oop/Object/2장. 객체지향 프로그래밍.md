...
클래스를 결정한 후에 클래스에 어떤 `속성`과 `메서드`가 필요한지 고민한다.

하지만 진정한 객체지향 패러다임으로의 전환은 **클래스가 아닌 객체에 초점을** 맞출 때에만 얻을 수 있다.

우리는 프로그래밍을 하는 동안 두 가지에 집중해야한다.

##### 1. 클래스가 아니라 어떤 객체들이 필요한지 고민하라.
...
![도메인을 구성하는 타입들의 구조](images/domain.png)![도메인 개념의 구조를 따르는 클래스 구조](images/class.png)
...
![](Pasted%20image%2020241017173838.png)
